...
func Parse(uri string) (q DdcQuery, err error) {
	// /ddc/org/my_org/buc/my_bucket/ifile/my_cid?option=yes
	uri = strings.TrimPrefix(uri, "/")

	// ddc/org/my_org/buc/my_bucket/ifile/my_cid?option=yes
	uri, options, _ := strings.Cut(uri, "?")
	q.Options = options

	// ddc/org/my_org/buc/my_bucket/ifile/my_cid
	parts := strings.Split(uri, "/")

	// [ddc org my_org buc my_bucket ifile my_cid]
	err = consumeDdc(&q, parts)
	if err != nil {
		return q, err
	}
...
func consumeDdc(q *DdcQuery, parts []string) error {
	if len(parts) < 1 || parts[0] != "ddc" {
